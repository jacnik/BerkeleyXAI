#Embedded file name: searchTestClasses.py
import re
import testClasses
import textwrap
import layout
...
class GraphSearch(SearchProblem):
    ...
    def __init__(self, graph_text):
        ...
        for l in lines[2:]:
            ...
            cost = float(cost)
            self.orderedSuccessorTuples.append((start,
             action,
             next_state,
             cost))
...
class GraphSearchTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if error != None:
            ...
            return False
        elif solution in gold_solution and (not self.exactExpansionOrder or expanded_states in gold_expanded_states):
...
class CornerProblemTest(testClasses.TestCase):
    ...
    def solution(self, search, searchAgents):
        ...
        missedCorners = [ p for p in ((1, 1),
         (1, top),
         (right, 1),
         (right, top)) if p not in visited ]
