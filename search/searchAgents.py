#Embedded file name: D:\AI\search\searchAgents.py
"""
...
def foodHeuristic(state, problem):
    ...
    px, py = position
    left = filter(lambda p: p[0] <= px, foodPoints)
    right = filter(lambda p: p[0] > px, foodPoints)
    tl = filter(lambda p: p[1] <= py, left)
    bl = filter(lambda p: p[1] > py, left)
    tr = filter(lambda p: p[1] <= py, right)
    br = filter(lambda p: p[1] > py, right)
    furthersWeights = []
    for group in [tl,
     bl,
     tr,
     br]:
        weights = [ (util.manhattanDistance(position, foodPos), foodPos) for foodPos in group ]
        if weights:
            maxWeight = max(weights, key=lambda a: a[0])
            furthersWeights.append(maxWeight)

    res = 0
    if furthersWeights:
        closeW = min(furthersWeights, key=lambda a: a[0])
        closeD, closeP = closeW
        furthersWeights.remove(closeW)
        res += closeD
        while furthersWeights:
            furthersWeights = [ (util.manhattanDistance(closeP, foodPos), foodPos) for w, foodPos in furthersWeights ]
            closeW = min(furthersWeights, key=lambda a: a[0])
            closeD, closeP = closeW
            res += closeD
            furthersWeights.remove(closeW)
